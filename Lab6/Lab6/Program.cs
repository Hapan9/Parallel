...
namespace Lab6
{
    internal class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            MultiProcessImmediateSample(values.ToArray());
            MultiProcessSample(values.ToArray());
        }
        ...
        private static void MultiProcessSample(string[] args)
        {
            MPI.Environment.Run(ref args, comm =>
            {
                ...
                if (comm.Rank == Master)
                {
                    Console.WriteLine("Blocking. Let's go.");
                    Console.WriteLine("Matrices rank - " + args[0]);
                    ...
                    //printMatrix(result);
                    //var expected = multiply(first, second);
                    //Console.WriteLine("Algorithm is correct - " + matricesEqual(expected, result));
                }
                else
                {
        ...
        private static void MultiProcessImmediateSample(string[] args)
        {
            MPI.Environment.Run(ref args, comm =>
            {
                ...
                else
                {
                    ...
                    var result = new MultiplyResultDto { Result = Multiply(data.FirstRows, data.Second), Offset = data.Offset };
